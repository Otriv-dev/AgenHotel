...
public class HospedeDAO {
    ...
    public List<Hospede> listar(String busca) throws SQLException {
        String sql = "SELECT id, nome, email, telefone FROM hospedes WHERE (? = '' OR nome LIKE ? OR email LIKE ? OR telefone LIKE ?) ORDER BY nome";
        ...
        try (Connection conn = banco.getConnection();
            ...
            for (int i = 2; i <= 4; i++) ps.setString(i, "%" + termo + "%");

            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) hospedes.add(montarHospede(rs));
            }
        }
        return hospedes;
    }
